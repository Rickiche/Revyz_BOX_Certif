...
page1 = None           # Page principale
page1 = None           # Page 2 (accessible via Fenetre1)
page2 = None           # Page 3 (accessible via Fen2)
...
def ouvrir_page2():
    ...
    barre_etat.showMessage("Page 2 ouverte", 2000)

def retourner_page1():
    """Retourne à la page principale"""
    global stacked_widget
    stacked_widget.setCurrentIndex(0)  # Index 0 = page1
...
if __name__ == "__main__":
    ...
    # ========== PAGE 1 : Page principale (contenu original) ==========
    page1 = QWidget()
    mise_en_page1 = QVBoxLayout()
    page1.setLayout(mise_en_page1)

    etiquette = QLabel("Entrez votre nom :")
    mise_en_page1.addWidget(etiquette)

    champ_nom = QLineEdit()
    champ_nom.setPlaceholderText("Votre nom ici")
    mise_en_page1.addWidget(champ_nom)

    bouton_saluer = QPushButton("Dire bonjour !")
    bouton_saluer.clicked.connect(on_click_saluer)
    mise_en_page1.addWidget(bouton_saluer)

    etiquette_resultat = QLabel("")
    etiquette_resultat.setAlignment(Qt.AlignCenter)
    mise_en_page1.addWidget(etiquette_resultat)

    mise_en_page1.addStretch()  # espaceur
    
    # Ajouter la page 1 au stacked widget (index 0)
    stacked_widget.addWidget(page1)

    # ========== PAGE 2 : Page accessible via Fenetre1 ==========
    page1 = QWidget()
    mise_en_page1 = QVBoxLayout()
    page1.setLayout(mise_en_page1)
    ...
    bouton_retour1.clicked.connect(retourner_page1)
    mise_en_page1.addWidget(bouton_retour1)

    mise_en_page1.addStretch()
    
    # Ajouter la page 2 au stacked widget (index 1)
    stacked_widget.addWidget(page1)
    ...
    bouton_retour2.clicked.connect(retourner_page1)
    ...
    # Menu Fen2 (direct, sans sous-menu) - CORRECTION ICI
    # On crée d'abord une action
    action_ouvrir_page2 = QAction("&Fenetre2", fenetre)
    action_ouvrir_page2.triggered.connect(ouvrir_page2)
    # Puis on ajoute l'action directement à la barre de menu
